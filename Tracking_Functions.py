...
#     Area[Area < 0] = 0
    ...
    col_buffer = 1500000 # m

    # check if cyclone is COL
    col_z500_objects = np.copy(cy_z500_objects)
    ...
    #         if min_z500_obj > np.min(col_region):
    #             ObjACT[tt,:,:] = 0
            if np.nanmin(z500_ACT_obj[tt]) > 100000:
                # there is no object to process
                ObjACT[tt,:,:] = 0
                continue
            # check if 700 km radius arround center has higher Z
            min_loc_tt = np.where(col_region[:,:] == np.nanmin(z500_ACT_obj[tt]))
            ...
            rdist = radialdistance(lat_reg[min_la_tt,min_lo_tt],
                                   ...
                                   lat_reg,
                                   lon_reg)
            ring = (rdist >= (700 - (dxObj/1000.)*2))  & (rdist <= (700 + (dxObj/1000.)*2))
            if min_z500_obj > np.min(col_region[ring]) - 30:
                ObjACT[tt,:,:] = 0
                continue
